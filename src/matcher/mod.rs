...
pub fn match_path(segments: &[path::Segment], text: &str) -> bool {
    ...
    match_path_recursive(segments, &chars, 0, 0)
}

fn match_path_recursive(segments: &[path::Segment], text: &[char], seg_idx: usize, t_idx: usize) -> bool {
        ...
        match &segments[seg_idx] {
            path::Segment::Literal(lit_chars) => {
                ...
                if t_idx + len <= text.len() && text[t_idx..t_idx + len] == lit_chars[..] {
                    match_path_recursive(segments, text, seg_idx + 1, t_idx + len)
                } else {
                    false
                }
            }
